...
1. delete all `.input2` files; (cd organize_me >>> rm *.input2)
2. remove the `flags/` directory and all of its contents; (rm -r flags)
3. place the test data text files in a directory named `test_data/`; (mkdir test_data/ >>> mv testdata* test_data/)
4. place the `.sh` files in a directory named `scripts_shell/`; (mkdir scripts_shell/ >>> mv *.sh scripts_shell/)
5. place the `.R` files in a directory named `scripts_R/`; (mkdir scrips.R/ >>> mv *.R scripts.R/)
6. place the `.qsub` files in a directory named `hpc_start/`;(mkdir hpc_start/ >>> mv *.qsub hpc_start/)
7. copy directory `organize_me/` and name the copy `simulations/`;(mkdir simulations/ >>> cp -r organize_me/ simulations/)
8. delete directory `organize_me/` and all of its contents. (rm -r organize_me/)
